//! Tauri commands for identity and mesh snapshot controls.
...
#[must_use]
pub(crate) fn hardware_harvest_enabled() -> bool {
    HARDWARE_HARVEST_ENABLED.load(Ordering::SeqCst)
}

/// Enable or pause hardware entropy harvesting for identity creation.
#[tauri::command]
...
pub fn generate_mnemonic_24() -> Result<Value, String> {
    if !hardware_harvest_enabled() {
        return Err("hardware harvest is paused".into());
    }
